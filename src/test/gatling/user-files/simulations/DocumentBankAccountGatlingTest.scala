...
/**
 * Performance test for the DocumentBankAccount entity.
 */
class DocumentBankAccountGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the DocumentBankAccount entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all documentBankAccounts")
            .get("/api/document-bank-accounts")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new documentBankAccount")
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_documentBankAccount_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created documentBankAccount")
                .get("${new_documentBankAccount_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created documentBankAccount")
            .delete("${new_documentBankAccount_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
