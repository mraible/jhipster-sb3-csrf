...
/**
 * Performance test for the EmbeddedOperation entity.
 */
class EmbeddedOperationGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the EmbeddedOperation entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all embeddedOperations")
            .get("/api/embedded-operations")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new embeddedOperation")
            .post("/api/embedded-operations")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_embeddedOperation_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created embeddedOperation")
                .get("${new_embeddedOperation_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created embeddedOperation")
            .delete("${new_embeddedOperation_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
