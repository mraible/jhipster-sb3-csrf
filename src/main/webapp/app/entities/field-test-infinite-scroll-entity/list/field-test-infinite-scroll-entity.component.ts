...
import { EntityArrayResponseType, FieldTestInfiniteScrollEntityService } from '../service/field-test-infinite-scroll-entity.service';
...
export class FieldTestInfiniteScrollEntityComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFieldTestInfiniteScrollEntity): string =>
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fieldTestInfiniteScrollEntities = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFieldTestInfiniteScrollEntity[] | null): IFieldTestInfiniteScrollEntity[] {
    const fieldTestInfiniteScrollEntitiesNew = this.fieldTestInfiniteScrollEntities ?? [];
    if (data) {
      for (const d of data) {
        if (fieldTestInfiniteScrollEntitiesNew.map(op => op.id).indexOf(d.id) === -1) {
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.fieldTestInfiniteScrollEntityService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
