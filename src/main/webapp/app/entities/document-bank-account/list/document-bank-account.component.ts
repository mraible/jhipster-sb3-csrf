...
import { EntityArrayResponseType, DocumentBankAccountService } from '../service/document-bank-account.service';
...
export class DocumentBankAccountComponent implements OnInit {
  ...
  trackId = (_index: number, item: IDocumentBankAccount): string => this.documentBankAccountService.getDocumentBankAccountIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.documentBankAccounts = this.refineData(dataFromBody);
  }

  protected refineData(data: IDocumentBankAccount[]): IDocumentBankAccount[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IDocumentBankAccount[] | null): IDocumentBankAccount[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.documentBankAccountService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
